...
I'm in an odd spot. After four years, I cling to a love for compilers. It's quite different from my AI gung-ho peers, matrix multiplication sitting in the holsters
...
Maybe it was my formative years where I mainly studied latin and greek, and fell in love with meticulously looking at essentially syntactically alien symbols and syntax, only to have my parents
...
equivalence, and found some rewrite branches to be better than others. As such, I've made a tiny stack machine in Lean. It has six instructions: push, add, mul, dup, pop, swap, a stack that holds symbolic expression trees rather than raw integers,
...
Originally, I thought to have the stack just hold integers. But comparing two programs of this sort boils down to a glorified list of numbers comparison, and there's nothing to prove. So I ended up having the stack hold symbolic Expr trees instead,
...
This project really uses a peephole optimiser as it walks through my bytecode-esque language, replacing short instruction sequences with provably equivalent shorter ones. Thus I have 'peep'. A chick makes the 'peep' sound, which is small,
